...
def FG_GetTrueBear(p_dLatObj, p_dlngObj, p_dLatTag, p_dlngTag):
    ...
    cosc = math.cos(PI / 2 - radLat2) * math.cos(PI / 2 - radLat1) + math.sin(PI / 2 - radLat2) * math.sin(
        PI / 2 - radLat1) * math.cos(radLng2 - radLng1)
...
def Mercator2LonLat(B, L, X, Y):
    ...
    for i in range(__IterativeValue):
        B = PI / 2 - 2 * math.atan(
            math.pow(E, (-X / K)) * math.pow(E, (e / 2) * math.log((1 - e * math.sin(B)) / (1 + e * math.sin(B)))))
...
def XYZ_To_BLH(original_long, original_lat, move_x, move_y, rotaionangle):
    ...
    Object_Long, Object_Lat = Mercator2LonLat(FG_degree2rad(original_lat), FG_degree2rad(original_long), mer_move_y,
                                              mer_move_x)
    return Object_Long, Object_Lat
...
if __name__ == '__main__':
    ...
    box = np.zeros((1, 4), dtype=float)
    box[0][-2] = object_long
    box[0][-1] = object_lat
    res = lonlat_to_xyz_batch(box, original_long, original_lat, rotaionangle)
    print("经纬转xy={}".format(res))
